...
assert_card_not_contains "$home" 'External article' 'article-card__primary-link'
assert_card_contains "$home" 'External article' 'class="article-card__external-destination" href="https://outside.example/articles/hugo" target="_blank" rel="external noopener noreferrer"'
...
assert_card_contains "$home" 'External article' '<svg class="icon icon--external" aria-hidden="true" viewBox="0 0 24 24"'
...
assert_not_contains "$stylesheet" '.article-card__primary-link'
...
assert_contains "$stylesheet" '.home-posts{max-width:var(--content-width);margin:0 auto}'
assert_contains "$stylesheet" '.article-featured--card{aspect-ratio:2/1;object-fit:contain}'
assert_not_contains "$stylesheet" '.article-featured--card{aspect-ratio:3/1'
assert_contains "$stylesheet" '.article-card__title a{color:var(--color-text);text-decoration:none}'
